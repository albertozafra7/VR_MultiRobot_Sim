...
public class SimplePathMover : MonoBehaviour {
	...
	void Update () {
        ...
        //SimulatePath(path.TTargets, t);

        if ((Target.position - path.TTargets[CurrentPoint].position).sqrMagnitude < 0.05)
            CurrentPoint++;
    ...
    public void SimulatePath(Transform[] Targets, float t)
    {
        if (CurrentPoint < Targets.Length - 1)
        {
            Target.position = Vector3.Lerp(Targets[CurrentPoint].position, Targets[CurrentPoint + 1].position, t);
            Target.rotation = Quaternion.Lerp(Targets[CurrentPoint].rotation, Targets[CurrentPoint + 1].rotation, t);
        }
    }
}
